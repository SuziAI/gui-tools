...
from src.auxiliary import ListCircle, BoxesWithType, is_point_in_rectangle, SetInt, SelectionMode, Colors, BoxProperty, \
    PieceProperties, box_property_to_color, get_image_from_box_fixed_size, open_file_as_tk_image, is_rectangle_big_enough, \
...
class MainWindow:
    def __init__(self, piece_properties: PieceProperties, weights_path: str):
        self.piece_properties: PieceProperties = None
        self.empty_image = None
        self.weights_path = weights_path  # weights path for segmentation algorithm

        class ProgramState:
            def __init__(self):
                self.images_dir = "./res/annotation_start_directory"  # Directory in which the images for the project are
                self.output_dir = "."  # Directory where the last file was saved
                self.initial_filename = "untitled"  # Filename of the last saved file

                self.current_image = None  # Currently displayed image
                self.image_name_circle: ListCircle = None  # ListCircle containing the list of images in the project folder and the currently selected one
                self.images = []  # The list of images which are active for the current project (i.e., the n pages the piece has)

                self.must_be_changed = None  # If this is true, the program state has changed and this triggers some actions TODO refactor
                self.type_dict = {}  # Dict saving the indices for the boxing belonging to the type. type_dict[TYPE] = [idx1, idx2, ...]
                self.notation_resources = NotationResources()  # TODO: must be changed since plugin dependent
                self.number_of_pages = SetInt(1)  # the number of image files the piece uses

        self.program_state = ProgramState()
        self.initialize(piece_properties)

    def initialize(self, program_state):
        self.piece_properties = program_state
        self.program_state.image_name_circle = ListCircle(get_folder_contents(self.program_state.images_dir, only_images=True))
        self.program_state.must_be_changed = False
        self.program_state.number_of_pages.set(self.piece_properties.number_of_pages)
    ...
    def exec(self):
        ...
        main_window = tk.Tk()
        main_window.title("Chinese Musical Annotation Tool- Main Window")
        ...
        def construct_image():
            self.piece_properties.base_image_path = self.program_state.image_name_circle.get_current()
            self.program_state.images = []

            if self.program_state.number_of_pages.get() > 0:
                for idx in range(0, self.program_state.number_of_pages.get()):
                    image_name = self.program_state.image_name_circle.get_nth_from_current(idx)
                    self.program_state.images.append(cv2.cvtColor(cv2.imread(image_name), cv2.COLOR_BGR2RGB))

                max_width, max_height = 0, 0
                for image in self.program_state.images:
                    max_height = max(image.shape[0], max_height)
                    max_width = max(image.shape[1], max_width)

                for idx in range(len(self.program_state.images)):
                    self.program_state.images[idx] = cv2.copyMakeBorder(
                        src=self.program_state.images[idx],
                        top=0,
                        bottom=max_height - self.program_state.images[idx].shape[0],
                        left=0,
                        right=max_width - self.program_state.images[idx].shape[1],
                        borderType=cv2.BORDER_CONSTANT,
                        value=[255, 255, 255]
                    )

                if invert_order.get():
                    self.program_state.images.reverse()

                self.program_state.current_image = cv2.hconcat(self.program_state.images)
                current_filename.set(get_current_filename_from_circle(self.program_state.image_name_circle))
                self.program_state.must_be_changed = False

        def ensure_current_image():
            image_must_be_changed = self.program_state.must_be_changed or self.piece_properties.base_image_path is None
            if image_must_be_changed:
                construct_image()

        def handle_opencv_window():
            ensure_current_image()
            self.piece_properties.content, exit_flag = opencv_window.draw_and_handle_clicks(self.piece_properties,
                                                                                            selectionmode_var.get(),
                                                                                            boxtype_var.get(),
                                                                                            self.program_state.current_image,
        ...
        def must_be_changed():
            self.program_state.must_be_changed = True
        ...
        def on_new_segmentation():
            ...
            def segment():
                if segment_pages_individually.get():
                    ...
                    boxes = []
                    for img in self.program_state.images:
                    ...
                    self.piece_properties.content.create_from_coordinate_list(
                        predict_boxes(self.program_state.current_image, self.weights_path))
        ...
        def on_previous():
            self.program_state.image_name_circle.previous()
            on_reset_segmentation()

        def on_next():
            self.program_state.image_name_circle.next()
            on_reset_segmentation()
        ...
        def create_sorted_segmentation_type_groups():
            ...
            for boxtype_var in dataclasses.astuple(BoxProperty()):
                try:
                    annotation_cursor[boxtype_var] = self.program_state.type_dict[boxtype_var].get_current()
                except KeyError:
                    annotation_cursor[boxtype_var] = None

                self.program_state.type_dict[boxtype_var] = []

            boxes = self.piece_properties.content
            for idx in range(len(boxes)):
                self.program_state.type_dict[boxes.get_index_type(idx)].append(idx)
                box_idx_to_image_dict[idx] = get_image_from_box_fixed_size(self.program_state.current_image, boxes.get_index_coordinates(idx))
            for boxtype_var in dataclasses.astuple(BoxProperty()):
                self.program_state.type_dict[boxtype_var] = ListCircle(self.program_state.type_dict[boxtype_var])

                if annotation_cursor[boxtype_var] is not None:
                    self.program_state.type_dict[boxtype_var].set_if_present(
        ...
        def set_current_annotation_index(idx: int):
            try:
                box_type = self.piece_properties.content.get_index_type(idx)
                boxtype_var.set(box_type)
                local_idx = self.program_state.type_dict[box_type].list.index(idx)
        ...
        def set_annotation_properties():
            ...
            def set_annotation_variables():
                ...
                current_box_is_excluded.set(self.piece_properties.content.is_index_excluded(current_idx))
                current_box_is_line_break.set(self.piece_properties.content.is_index_line_break(current_idx))
        ...
        def access_current_segmentation_cycle() -> ListCircle:
            return self.program_state.type_dict[boxtype_var.get()]
        ...
        def on_save_annotation_to_box(*args):
            ...
            if selectionmode_var.get() == SelectionMode.ANNOTATE:
                ...
                self.piece_properties.content.set_index_annotation(get_current_annotation_index(), annotation_string)
                self.piece_properties.content.set_index_excluded(get_current_annotation_index(), current_box_is_excluded.get())
                self.piece_properties.content.set_index_line_break(get_current_annotation_index(), current_box_is_line_break.get())

        def on_fill_all_boxes_of_type(text: str):
            if selectionmode_var.get() == SelectionMode.ANNOTATE:
                if boxtype_var.get() == BoxProperty.MUSIC:
                    for idx, box_index in enumerate(self.program_state.type_dict[boxtype_var.get()].list):
        ...
        def on_save_image():
            ...
            if draw_image is not None:
                file_path = asksaveasfilename(
                    initialdir=self.program_state.output_dir,
                    initialfile=get_current_filename_from_circle(self.program_state.image_name_circle),
        ...
        def get_content_list(key: str):
            try:
                return [self.piece_properties.content.get_index_annotation(box_idx) for box_idx in
                        self.program_state.type_dict[key].list]
            except AttributeError:
                return None

        def get_box_images(key: str):
            try:
                coordinates = [self.piece_properties.content.get_index_coordinates(box_idx) for box_idx in self.program_state.type_dict[key].list]
                images = [get_image_from_box_ai_assistant(self.program_state.current_image, coord) for coord in coordinates]
                return images
            except AttributeError:
                return None
        ...
        def get_line_break_indices(key):
            raw_indices = self.piece_properties.content.get_line_break_indices()
            box_idx_list = self.program_state.type_dict[key].list
            line_break_idxs = []
        ...
        def get_notation_image():
            ...
            if display_notes_frame.is_jianpu():
                notation_img = notation_to_jianpu(self.program_state.notation_resources.small_font,
                                                  self.program_state.notation_resources.jianpu_image_dict,
                                                  music_list, lyrics_list,
                                                  line_break_idxs,
                                                  fingering)
            else:
                notation_img = notation_to_western(self.program_state.notation_resources.small_font,
                                                   self.program_state.notation_resources.western_image_dict,
                                                   music_list, lyrics_list,
                                                   line_break_idxs,
                                                   fingering)
        ...
        def get_complete_notation_image():
            ...
            metadata_img = construct_metadata_image(self.program_state.notation_resources.title_font,
                                                    self.program_state.notation_resources.small_font,
        ...
        def on_save_notation():
            ...
            if draw_image is not None:
                file_path = asksaveasfilename(
                    initialdir=self.program_state.output_dir,
                    initialfile=self.program_state.initial_filename,
                    defaultextension=".png",
        ...
        def on_save():
            self.piece_properties.number_of_pages = self.program_state.number_of_pages.get()

            self.piece_properties.mode_properties = annotation_frame.get_mode_properties()
            new_program_state = copy.copy(self.piece_properties)
            ...
            if file_path:
                with open(file_path, "w") as json_file:
                    ...
                    for idx in range(json_state["number_of_pages"]):
                        image_list.append(os.path.relpath(self.program_state.image_name_circle.get_nth_from_current(idx), start=os.path.dirname(file_path)))  # we must save the relative path without modifying the program state
        ...
        def on_save_text():
            ...
            if file_path:
                def get_content_string(key):
                    create_sorted_segmentation_type_groups()
                    try:
                        raw_content_list = [self.piece_properties.content.get_index_annotation(box_idx) for box_idx in self.program_state.type_dict[key].list]
        ...
        def on_new():
            ...
            if len(get_folder_contents(images_dir, only_images=True)) == 0:  # no images in folder!
                ...
                self.program_state.images_dir = images_dir
                self.program_state.output_dir = images_dir
                self.program_state.initial_filename = "untitled"
                self.initialize(program_state)
                right_increment_decrement_widget.set_counter(self.program_state.number_of_pages.get())
                mode_string.set(GongdiaoModeList.from_properties(self.piece_properties.mode_properties).name)

                annotation_frame.set_mode_properties(self.piece_properties.mode_properties)

                self.program_state.image_name_circle.set_if_present(os.path.join(self.program_state.images_dir, Path(program_state.base_image_path).name))
                must_be_changed()
                on_activate_deactivate_annotation_frame()
        ...
        def on_load():
            ...
            if file_path:
                with open(file_path, "r") as json_file:
                    ...
                    program_state = PieceProperties.load(json_contents)
                    self.program_state.images_dir = os.path.join(os.path.dirname(file_path), os.path.dirname(program_state.base_image_path))
                    self.program_state.output_dir = os.path.dirname(file_path)
                    self.program_state.initial_filename = Path(file_path).stem
                    self.initialize(program_state)
                    right_increment_decrement_widget.set_counter(self.program_state.number_of_pages.get())
                    mode_string.set(GongdiaoModeList.from_properties(self.piece_properties.mode_properties).name)
        ...
        right_increment_decrement_widget = IncrementDecrementFrame(increment_decrement_subframe, self.program_state.number_of_pages, must_be_changed)
        ...
        def handle_additional_info():
            ...
            def get_suzipu_list():
                try:
                    suzipu_list = [self.piece_properties.content.get_index_annotation(box_idx) for box_idx in
                                   self.program_state.type_dict[BoxProperty.MUSIC].list]
        ...
        def on_activate_deactivate_annotation_frame():
            state = selectionmode_var.get() == SelectionMode.ANNOTATE and selection_buttons.is_active and len(self.program_state.type_dict[boxtype_var.get()].list) > 0
